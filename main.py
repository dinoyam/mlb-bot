...
# Render web services must bind the port in $PORT or the deploy is marked
# failed. This thread does nothing but answer health checks.
...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"mlb bot alive")
    ...
    def log_message(self, *args):
        # Silence per-request logging so the Render log stays readable.
        pass
...
print("⚾ Home Run Bot is running live...", flush=True)
...
def get_play_key(game_pk, play):
    """Stable per-play identifier.

    `about.playId` does not exist on a play object (playId lives on individual
    playEvents), so the original version produced "<game_pk>_None" for every
    play and only ever alerted on the first home run of a game. atBatIndex is
    unique per plate appearance within a game.
    """
    about = play.get("about", {})
    index = about.get("atBatIndex")

    if index is None:
        index = play.get("atBatIndex")

    if index is None:
        # Last resort: fall back to something that at least varies per play.
        index = about.get("startTime") or about.get("endTime")

    return f"{game_pk}_{index}"
        ...
        for game in games:
            ...
            if status == "Live":
                try:
                    ...
                    is_first_look = previous_scores is None
                    ...
                    for play in reversed(all_plays):
                        ...
                        if result.get("event") == "Home Run":
                            play_id = get_play_key(game_pk, play)

                            if play_id in sent_alerts:
                                continue

                            # On the very first look at a game that is already
                            # in progress, record past homers without firing an
                            # alert for each one. Prevents a burst of stale
                            # alerts after a Render restart mid-game.
                            if is_first_look:
                            ...
                            response = requests.post(
                                DISCORD_WEBHOOK_URL,
                                json={"content": get_home_run_message(feed, play)},
                                timeout=10,
                            )
                            response.raise_for_status()
                            sent_alerts.add(play_id)
                    ...
                    print(
                        f"Error processing game {game_pk}: {game_error}",
                        flush=True,
                    )
                    continue

    except Exception as error:
        print(f"Error checking scores: {error}", flush=True)
